...
app = Flask(__name__)
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8967099088:AAEOM19D8pK0VU2n0NzJpZinLkbt8alK19w")
...
def index():
    ...
    <body>
        ...
        <div class="box">
            <h3>1. /start & ምዝገባ</h3>
            <div class="info-box" id="userInfo">ቴሌግራም ID በመፈለግ ላይ...</div>
            <input type="text" id="phone" placeholder="ስልክ ቁጥር (ለምሳሌ 0923...)">
            <button class="btn" onclick="registerUser()">ስልክ ቁጥር አስመዝግብ</button>
        </div>
        ...
        <div class="box">
            ...
            <input type="text" id="smsText" placeholder="የባንክ/ቴሌብር የክፍያ SMS መልእክት እዚህ ይለጥፉ">
            <button class="btn" onclick="makeDeposit()">ክፍያ አረጋግጥና ላክ</button>
        </div>

        <!-- ገንዘብ ማውጣት (Withdraw) -->
        <div class="box">
            <h3>5. /withdraw (ገንዘብ ማውጣት)</h3>
            <p style="font-size:11px; color:#e67e22;">ማስታወሻ፡ 50 ብር ቀሪ መኖር አለበት። ከፍተኛው ውጭ 2000 ብር ነው።</p>
        ...
        <script>
            ...
            window.onload = function() {
                if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initDataUnsafe && window.Telegram.WebApp.initDataUnsafe.user) {
                    ...
                    telegramId = 12345678;
                    document.getElementById('userInfo').innerText = "Test Mode ID: 12345678 (ቴሌግራም ውጪ)";
                    loadAccount();
                }
            };
            ...
            function registerUser() {
                ...
                fetch('/api/register', {
                    ...
                    body: JSON.stringify({telegram_id: telegramId, phone_number: phone})
                })
                .then(res => res.json())
            ...
            function makeDeposit() {
                ...
                if(!amount) return alert('እባክዎ መጠን ይሙሉ');

                fetch('/api/deposit', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({telegram_id: telegramId, amount: parseFloat(amount), method: method, sms_text: sms})
                })
                .then(res => res.json())
                .then(data => { alert(data.message); loadAccount(); });
            }
            ...
            function makeWithdraw() {
                ...
                if(!amount || !acc) return alert('ሁሉንም መረጃዎች ይሙሉ');

                fetch('/api/withdraw', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({telegram_id: telegramId, amount: parseFloat(amount), method: method, account: acc})
                })
                .then(res => res.json())
                .then(data => { alert(data.message); loadAccount(); });
            }
            ...
            function startPlay() {
                ...
                let timerInterval = setInterval(() => {
                    ...
                    if(timeLeft < 0) {
                        ...
                        timerElem.innerText = "🎮 ጨዋታው ተጀምሯል!";
                        fetchAndRunGame(stake);
                    }
                }, 1000);
            }

            function fetchAndRunGame(stake) {
                fetch('/api/play', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({telegram_id: telegramId, stake: stake})
                })
                .then(res => res.json())
                .then(data => {
            ...
            function simulateBingoCaller(cardNumbers) {
                ...
                let interval = setInterval(() => {
                    if(calledCount >= 8 || cardNumbers.length === 0) {
                        clearInterval(interval);
                        document.getElementById('callStatus').innerText = "🎉 ቢያንጎ! (BINGO) አሸንፈዋል!";
                        loadAccount();
                        return;
                    }
                    let randNum = cardNumbers[Math.floor(Math.random() * cardNumbers.length)];
                    document.getElementById('callStatus').innerText = `🔊 የተጠራ ቁጥር: ${randNum}`;
                    let cell = document.getElementById('cell-' + randNum);
            ...
            function transferMoney() {
                ...
                if(!phone || !amount) return alert('መረጃዎችን ይሙሉ');

                fetch('/api/transfer', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({telegram_id: telegramId, target_phone: phone, amount: parseFloat(amount)})
                })
                .then(res => res.json())
                .then(data => alert(data.message));
            }
...
def deposit():
    ...
    conn.commit()
    conn.close()
    return jsonify({"status": "success", "message": f"ክፍያው ተረጋግጧል! {amount} ብር ገብቷል።"})
...
def withdraw():
    ...
    if not row or (row[0] - amount) < 50:
        conn.close()
        return jsonify({"status": "error", "message": "ውድቅ ተደርጓል! ቢያንስ 50 ብር ቀሪ ሊኖር ይገባል።"})
        
    if amount > 2000:
        conn.close()
        return jsonify({"status": "error", "message": "ከፍተኛው የማውጣት ገደብ 2000 ብር ነው።"})
